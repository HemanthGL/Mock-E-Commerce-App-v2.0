...
export function cardForCartProd(product: StoredProd):string{

    let truncatedTitle = product.product.title.substring(0, 35);

    let cardHTML: string = `
            <div class="card" style="min-width: 220px" >
                <div id="card-${product.product.id}" class="bubble"> 
                    <div style="object-fit: contain">
...
export function cardForArrayCart(arr: Array<StoredProd>): string{

    let cardsHTML: string = '';

    arr.forEach((ele: StoredProd) => {
